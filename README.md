...
* [`src/clj/re_frame_sandbox/`](src/clj/re_frame_sandbox/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/re_frame_sandbox/`](src/cljs/re_frame_sandbox/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/re_frame_sandbox/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `re-frame-sandbox` project root directory
...
in the [`src/clj/re_frame_sandbox/`](src/clj/re_frame_sandbox/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/re_frame_sandbox/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns re-frame-sandbox.example
  (:require [re-frame-sandbox.config :as config])
